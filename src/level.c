...
// Level data ported from docs/doom-nano/level.h (based on E1M1 from
// Wolfenstein 3D). Two cells per byte, 4 bits each; rows are stored bottom
// up, which is why getBlockAt() inverts y.
//
// Legend: # wall, . floor, P player, E enemy, D door, L locked door,
//         X exit, M medikit, K key
//
//   ################################################################
//   #############################...........########################
//   ######....###################........E..########################
...
const uint8_t level_data[LEVEL_SIZE] = {
...
/**
 * Read the 4-bit cell at (x, y) out of the packed level data.
 */
uint8_t getBlockAt(const uint8_t level[], uint8_t x, uint8_t y) {
    ...
    // y is stored in inverse order
    return (level[((LEVEL_HEIGHT - 1 - y) * LEVEL_WIDTH + x) / 2]
            >> (!(x % 2) * 4))   // select the high or low nibble
           & 0x0F;
}
